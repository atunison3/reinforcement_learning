...
This project studies two distinct ideas in action-value methods: optimistic initial values as a mechanism for exploration, and constant step sizes as a mechanism for tracking nonstationary rewards. In a drifting bandit problem, the agent must both discover good actions and keep adapting after the identity of the best action changes.
...
The source file is `reinforcement_learning/projects/project004.py`. It defines a nonstationary $k$-armed bandit in which each run begins with true action values drawn from $\mathcal{N}(0,1)$. After every step, all true values drift according to independent Gaussian increments with standard deviation $0.01$. Rewards are sampled from $\mathcal{N}(q_*(a),1)$.
...
![Average reward over time for 10 arms](assets/project_4_average_reward_k10.png)
...
![Optimal action percentage over time for 10 arms](assets/project_4_optimal_action_percentage_K10.png)
...
![Average reward over time for 20 arms](assets/project_4_average_reward_k20.png)
...
![Optimal action percentage over time for 20 arms](assets/project_4_optimal_action_percentage_K20.png)
