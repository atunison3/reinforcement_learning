...
Completed configuration/runs: **36 / 36**.
Each complete plotted setting averages all of the first **100,000 episodes per run**, across **1 independent run(s)**.
Master seed: `42`; fixed map seed: `42`; horizon: `500` turns; learning-curve block: `1000` episodes.
Source/settings signature: `8afae51683d2322e1ea380971c11e2d10c57c1d9288f48670a3ff05c9c2c4dc1`.
...
This is a **single-run-per-setting study**: no uncertainty bands are drawn. Episode samples within one learning run are not treated as independent replications.

### Best completed sampled settings

| Method | Parameter | Mean episode return | Success | Death | Timeout | Mean turns on success |
| --- | ---: | ---: | ---: | ---: | ---: | ---: |
| Epsilon-greedy | 0.03125 | 74.0403 | 98.90% | 1.06% | 0.04% | 13.58 |
| Gradient bandit → episodic REINFORCE | 1 | 57.9276 | 97.77% | 2.11% | 0.11% | 16.93 |
| Greedy, optimistic initialization | 0.015625 | 40.2494 | 96.98% | 2.12% | 0.91% | 21.67 |
...
- State-local UCB spans -159.7467 (parameter 4) to 65.8928 (0.0078125) across completed settings.
...
```bash
python -m reinforcement_learning.projects.project011 --study --episodes 100000 --runs 1 --seed 42 --map-seed 42 --max-turns 500 --block-size 1000 --parameters 0.0078125 0.015625 0.03125 0.0625 0.125 0.25 0.5 1.0 2.0 4.0 --output-dir "docs/projects/assets/project_11"
```
